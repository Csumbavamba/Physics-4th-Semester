...
Camera::Camera(ViewMode viewMode)
{
	...
	cameraPosition = glm::vec3(0.0f, +100.0f, 0.0f);
...
{
	projectionMatrix = (glm::perspective(120.0f, /*(float)SCREEN_WIDTH*/(float)Utility::GetScreenWidth() / (float)Utility::GetScreenHeight(), 0.1f, 15000.0f));
}
...
void Camera::RotateAroundObject(glm::vec3 objectLocation, float distanceFromObject, float deltaTime)
{
	...
	float yMovement = cos(timeElapsed) * distanceFromObject;
	float cameraY = 0.5f * distanceFromObject;

	cameraUpDirection = glm::vec3(0.0, 1.0, 0.0);
	cameraPosition = glm::vec3(objectLocation.x + xMovement, cameraY, objectLocation.z + yMovement);
